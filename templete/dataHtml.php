  ...
  <div data-role="page" data-title="電話対応虎の巻" id="page1" data-url="page1" tabindex="0" class="ui-page ui-page-theme-c ui-page-footer-fixed ui-page-active" style="padding-bottom: 58px;">
      ...
      <div role="main" class="ui-content" id="content">
        ...
        ?>        
          ...
          <p>
            <?php if(mb_strlen($data[0]["cus_memo"])>0 ||$data[0]["cus_memo"]!=NULL){echo "<span>メモ:</span>".$data[0]["cus_memo"];} ?>
            <br>
            <?php if($data[0]["err_flag"]==0){echo "";}else{echo "<span>エラーフラグ:</span>以前聞き間違えました･･･";} ?>
          </p>
        </div>
      </div>
      ...
      <!-- サイドパネル部分書き換え中 -->
        ...
        <h3>情報更新</h3>
        <form name="edit" action="../htdocs/editdata_comp.php" method="POST" id="edit">
          <br>
          <h4>社員の情報更新</h4>
          ...
          <br>
          <h4>退職した際の情報更新</h4>
          <span>現在の第一担当</span><input type="text" value="<?php if(mb_strlen($data[0]["now_in_fir"])>0){echo $data[0]["now_in_fir"];} ?>" name="now_in_fir">
          <span>現在の第二担当</span><input type="text" value="<?php if(mb_strlen($data[0]["now_in_sec"])>0){echo $data[0]["now_in_sec"];} ?>" name="now_in_sec">
          <br>
          <h4>お客様先の情報更新</h4>
          <span>メモ</span><textarea name="cus_memo"><?php if(mb_strlen($data[0]["cus_memo"])>0){echo $data[0]["cus_memo"];} ?></textarea>
            <input type="hidden" name="emp_id" value="<?php echo $data[0]["emp_id"]; ?>">
            <input type="hidden" name="empinfo_id" value="<?php echo $_POST["empinfo_id"]; ?>">
            <input type="hidden" name="cus_id" value="<?php echo $data[0]["cus_id"]; ?>">
